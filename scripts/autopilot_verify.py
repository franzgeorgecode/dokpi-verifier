...
def inspect_security(cwd):
    ...
    if rendered.returncode != 0:
        raise RuntimeError(
            "compose_security_render_failed"
        )
    ...
    try:
        ...
        if (
            ...
            try:
                ...
                if not db_id or not app_id:
                    raise RuntimeError(
                        "compose_container_id_missing"
                    )
                ...
                if restore.returncode != 0:
                    raise RuntimeError("psql_restore_failed")
                ...
                if (
                    db_value != "before-backup" or
                    file_value != "before-backup"
                ):
                    raise RuntimeError(
